...
{
    FileSize = size;
    emit Extractor::getInstance()->extract_filesize_signal(FileSize);
    return S_OK;
}
...
#endif
    emit Extractor::getInstance()->extract_completeValue_signal(*completeValue);
    return S_OK;
}
...
STDMETHODIMP CArchiveExtractCallback::PrepareOperation(Int32 askExtractMode)
{
    ...
    case NArchive::NExtract::NAskMode::kExtract:
        ...
        emit Extractor::getInstance()->extracting_filename_signal(filepath);
        break;
...
{
    FileSize = size;
    emit Extractor::getInstance()->compress_filesize_signal(FileSize);
    return S_OK;
}
...
#endif
    emit Extractor::getInstance()->compress_completeValue_signal(*completeValue);
    return S_OK;
}
...
#endif
    emit Extractor::getInstance()->compressing_filename_signal(QString::fromWCharArray(name));
}
...
Extractor::Extractor(QObject *parent) :
    QObject(parent),
    m_7zLib(kDllName) {
    qRegisterMetaType<Extractor::Operation>();
...
int Extractor::init(void)
{
    ...
    if (true == loadResult){
#ifdef DEBUG_LOGOUT_ON
        qDebug() << "Extractor" << kDllName << "load success.";
#endif
        init_result = 0;
    }
    else{
        qDebug() << "Extractor" << kDllName << "load failure!!!";
        init_result = 1;
    }
...
int Extractor::extract(const QString &archive_name, const QString &output_path) {
...
void Extractor::threadStarted(void) {
    int init_result = init();
    emit threadStarted(init_result);
}

void Extractor::onExtract(const QString archive_name, const QString output_path) {
    int operate_result = 0;

    operate_result = extract(archive_name, output_path);

    emit operationResult(archive_name, operate_result);
}
